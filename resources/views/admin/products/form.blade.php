            ...
            <form action="{{ isset($product) ? route('admin.products.update', $product) : route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="weight" value="{{ $product->weight ?? old('weight') }}" min="0"
                            @class([
                                'w-full px-4 py-2 border rounded focus:ring-2 focus:ring-blue-500',
                                'border-red-500' => $errors->has('weight'),
                                'border-gray-300' => !$errors->has('weight'),
                            ])>
                        <p class="text-sm text-gray-500 mt-1">Fill weight in grams (e.g., 500 for 0.5kg). Leave empty to use default shipping weight.</p>
                        @error('weight')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
